...
class GenCodeSysPrompt:
    ...
    __prompt: str = '''
    你是一个代码生成助手, 可以实现以下功能(如果用户没有特别指定, 则生成代码优先为 {code_type}, 第三方安装依赖工具优先为 {install_tool}):
...
class GenCodePrompt:
    ...
    输出格式说明:
        ...
        3. 代码实现: <gen_code></gen_code> 表示安装用户输入的需求分析, 实现的满足需求的代码(不需要任何如如markdown ```之类的编程语言占位符号)
        4. 测试代码: <test_code></test_code> 表示可以在其它文件下测试生成代码的测试文件代码(不需要任何如如markdown ```之类的编程语言占位符号)
